...
const OurServices: React.FC = ({ ourServices }) => {
	...
	useEffect(() => {
		async function getCmsData() {
			...
			if (topLabel !== "off") {
				setTopLabel(ourServices.topLabel);
	...
	return (
		<div className="w-full h-screen">
			...
			{ourServices?.hero && (
				<div
					className={`relative w-full bg-hero-texture-mobile lg:bg-hero-texture bg-cover bg-bottom bg-no-repeat ${topLabel && "mt-[150px] lg:mt-[78px]"
						}`}
				>
			...
			{ourServices?.masteries && (
				<SectionContainer className="pb-[0] lg:pb-[0]">
					{ourServices.masteries.map((mastery, index) => (
						...
						>
							<div
								className={`flex justify-center w-full min-h-[60px] md:min-h-[138px] pt-[7px] md:pt-[28px] text-[28px] leading-[32px] md:text-[56px] md:leading-[60px] text-[#F7F9FC] font-bold ${mastery.type === "tax"
