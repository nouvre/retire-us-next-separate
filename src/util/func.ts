...
export const useQuestionnaire = ({ answers }: { answers: Answer }) => {
  ...
  const checkRetRF4 = React.useCallback(() => {
    if (answers.step21Answer_1 == 1 || answers.step22Answer_2 == 1) return true;
    else return false;
  }, [answers]);

  const checkRetRF5 = React.useCallback(() => {
    if (answers.step21Answer_3 == 1 || answers.step21Answer_4 == 1) return true;
    else return false;
  }, [answers]);
  ...
  const checkCfpRF1 = React.useCallback(() => {
    if (answers.step11Answer_5 == 1 || answers.step16Answer_3 == 1 || answers.step16Answer_4 == 1)
      return true;
    else return false;
  }, [answers]);
  ...
  const checkTaxRF4 = React.useCallback(() => {
    if (answers.step40Answer_2 == 1 || answers.step40Answer_3 == 1) return true;
    else return false;
  }, [answers]);

  const checkTaxRF5 = React.useCallback(() => {
    if (answers.step6Answer_4 == 1 || answers.step33Answer_2 == 1) return true;
    else return false;
  }, [answers]);
  ...
  const getTaxVal = React.useCallback(() => {
    ...
    if (answers?.wantToRetireAge < 59 || answers?.wantToRetireAge > 65) val += 1;
    if (num(answers?.step12Answer_1) === 1 && num(answers?.step10Answer_2) === 1) val += 1;
    if (num(answers?.step12Answer_1) === 1 && num(answers?.step10Answer_1) === 1) val += 1;
    return val;
  }, [answers]);
  ...
  const getRedFlagList = React.useCallback(() => {
