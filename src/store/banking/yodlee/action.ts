...
export const generateToken: AppThunk = () => {
  return async (dispatch: Dispatch) => {
    ...
    return await axios
      .post(yodleeTokenUrl, params, config)
      .then(({ data }) => {
        if (data) {
          dispatch({
...
export const yodleeSuccess: AppThunk = (
  data: YodleeFirstStepResult,
  token: string
) => {
...
export const saveTokens: AppThunk = (public_tokens) => {
  return async (dispatch: Dispatch) => {
    return await axios
      .post("yodlee/set-yodlee-access-token", { public_tokens })
      .then(({ data }) => {
        dispatch({
