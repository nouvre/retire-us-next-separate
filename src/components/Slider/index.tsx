...
const CustomSlider = ({ items }: { items: Array<SliderType> }) => {
	...
	return (
		<div className="w-full relative overflow-hidden">
			...
			<div
				className={`flex justify-between items-center gap-10 absolute top-0 ${option.duration ? "duration-500" : ""
					}`}
				style={{
					...
					data.map((item, key) => {
						return (
							...
							>
								{key === 2 && !option.isMove && (
									<p
										className={`text-base md:text-xl text-transparent ${item.pretitle
								...
								{key === 2 && item.cta && !option.isMove && (
									<a
										href={item.cta.link}
										target={"_blank"}
										className="text-base md:text-lg text-[#001F55] flex items-center gap-3" rel="noreferrer"
									>
										{item.cta.text}
										<Image
